...
import {
  isUnicodeUrl,
} from "github-schema/utils";

const BANNER = `// THIS FILE IS GENERATED AUTOMATICALLY. DO NOT EDIT.
// RUN \`npm run update:emojis\` TO UPDATE.
`;

function getCategories(fullEmojiList: string) {
  const categories: string[] = [];

  const lines = fullEmojiList.split("\n");

  for (const line of lines) {
    if (line.startsWith("# group:")) {
      const category = line.slice("# group: ".length);
      categories.push(category);
    }

    if (line.startsWith("# subgroup:")) {
      const category = line.slice("# subgroup: ".length);
      categories.push(category);
    }
  }
...
async function run() {
  ...
  const fullEmojiList = await fetch("https://unicode.org/emoji/charts/full-emoji-list.txt").then((res) => res.text());
